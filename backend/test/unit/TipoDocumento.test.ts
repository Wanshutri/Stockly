...
// Códigos SII usados para las pruebas
const CODIGO_BASE = 30;
const CODIGO_OTRO = 33;

describe('TipoDocumentoTributario API', () => {

    // Se asume que existe una tabla dependiente 'documento_tributario'
    // para probar la restricción de clave foránea.
    beforeEach(async () => {
        ...
        // No se requiere reiniciar secuencias ya que codigo_sii es PRIMARY KEY y no SERIAL.
    });

    // Función auxiliar para crear un tipo de documento
    const createTipoDocumento = async (codigo: number, nombre: string) => {
        return request(app)
    ...
    describe('POST /api/tipos-documento', () => {
        ...
        it('Debe devolver un error 400 si falta el codigo_sii', async () => {
            // Se asume que el controlador también valida la presencia de codigo_sii
            const response = await request(app)
                .post('/api/tipos-documento')
                .send({ nombre_tipo: 'Factura Faltante' });

            expect(response.status).toBe(400);
            // El mensaje de error debe reflejar la validación del código SII
            // Nota: Este mensaje debe coincidir con el del controlador.
    ...
    describe('DELETE /api/tipos-documento/:codigo_sii', () => {
        ...
        it('Debe devolver un error 409 al intentar eliminar un código con dependencias (FK)', async () => {
            // 1. Crear el tipo de documento tributario
            await createTipoDocumento(CODIGO_BASE, 'ConDependencia');

            // 2. Simular la dependencia: Insertar una fila en la tabla dependiente
            // Se asume que 'documento_tributario' tiene una columna 'codigo_sii' que referencia a 'tipo_documento_tributario'
            await query('INSERT INTO documento_tributario (codigo_sii) VALUES ($1)', [CODIGO_BASE]);

            // 3. Intentar eliminar
            const deleteResponse = await request(app).delete(`/api/tipos-documento/${CODIGO_BASE}`);
            
            // Esperar el código 409 Conflict
            expect(deleteResponse.status).toBe(409);
            expect(deleteResponse.body.message).toBe('No se puede eliminar el Tipo de Documento Tributario porque todavía existen referencias a este código.');
        });
